...
function EditProductModalContent({ product, categories, onClose }: EditProductModalContentProps) {
  ...
  const [hasSizeVariants, setHasSizeVariants] = useState<boolean>(() => {
    const hasVariants = product.variants && product.variants.length > 0;
    if (hasVariants) {
      const distinctSizes = Array.from(new Set(product.variants!.map(v => v.size).filter(Boolean)));
      return distinctSizes.length > 1 || (distinctSizes.length === 1 && distinctSizes[0] !== "S");
    }
    return false;
  });
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <input
                      ...
                      min="0.01"
                      step="0.01"
                      defaultValue={product.base_price || product.price}
                      placeholder="0.00"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="0.01"
                      step="0.01"
                      defaultValue={product.discounted_price || product.price}
                      placeholder="0.00"

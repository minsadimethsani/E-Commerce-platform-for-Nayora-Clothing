    ...
    <div className="min-h-screen bg-neutral-50 flex font-sans">
      ...
      <aside className="w-64 bg-white border-r border-neutral-200 hidden md:flex flex-col h-screen sticky top-0 z-30">
        ...
        <nav className="flex-1 px-4 mt-6 space-y-1">
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                <link.icon className={`w-5 h-5 mr-3 ${isActive ? "text-white" : "text-neutral-400"}`} />
                {link.label}
              </Link>
            );
          })}
        ...
        <div className="p-4 border-t border-neutral-200 relative" ref={profileRef}>
          ...
          >
            <img
              className="h-10 w-10 rounded-full object-cover border border-neutral-200 group-hover:border-neutral-300 transition-colors"
              src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80"
              alt="Admin Profile"
            />
            <div className="ml-3 flex-1 flex flex-col items-start overflow-hidden">
              <span className="text-sm font-semibold text-neutral-900 truncate w-full">Admin User</span>
              <span className="text-xs text-neutral-500 truncate w-full">Super Admin</span>
            </div>
          ...
          {isProfileOpen && (
            <div className="origin-bottom-left absolute left-4 bottom-20 w-56 rounded-xl shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none animate-in slide-in-from-bottom-2">
              <div className="px-4 py-3 border-b border-neutral-100">
                <p className="text-sm text-neutral-900 font-medium">admin@nayora.com</p>
              </div>
              <Link
                href="/admin/settings"
              ...
              <button
                ...
                onClick={() => { setIsProfileOpen(false); alert('Logout feature coming soon'); }}
              >
